...
class CustomDescriptionFormatter(argparse.RawTextHelpFormatter):
    def _get_help_string(self, action):
        help = action.help
        if "(default" not in help and type(action) not in (
...
class BaseParser(argparse.ArgumentParser):
    ...
    def _check_value(self, action, value):
        if (
            action.choices is not None and value not in action.choices
        ):  # special handling for a nicer "choice" error message
...
class AutoDiscoveryCommandMap(object):
    ...
    def parse_help(self, module):
        ...
        if doc is None:
            return (
                NO_HELP,
                "No description provided, you could add one! Code is probably located here: %s"
    ...
    def build(self, force_autodiscover=False, break_if_unavailable=False):
        ...
        cached_command_map_filename = os.getenv(
            "%s_COMMANDS_CACHE" % (self.cligraph.conf.tool.shortname.upper()),
        ...
        if os.access(os.path.dirname(cached_command_map_filename), os.W_OK) and (
            not os.path.exists(cached_command_map_filename) or os.access(cached_command_map_filename, os.W_OK)
        ):
